...
public class WarBaseBrainController extends WarBaseAbstractBrainController {
	...
	public String action() {
		...
		ArrayList<WarMessage> MessExplo = getBrain().getMessages();
		
		for (int i=0;i<MessExplo.size();i++)
		{
			if (MessExplo.get(i).getMessage().equals(Constants.enemyBaseHere))
			{
				//messageFoodExplorer = true;
				System.out.println(MessExplo.get(i).toString());
				String[] pos = MessExplo.get(i).getContent();
				getBrain().broadcastMessageToAgentType(WarAgentType.WarRocketLauncher,Constants.enemyBaseHere,pos);
				//pos = new Double(angle[0]).doubleValue();
