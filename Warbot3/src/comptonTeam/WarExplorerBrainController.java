...
public class WarExplorerBrainController extends WarExplorerAbstractBrainController {
	...
	public String action() {
		...
		boolean messageFoodExplorer = false;
		
	
		WarMessage m = this.getMessageAboutFood();
		
		
		if (m != null)
		{
			messageFoodExplorer = true;
			String angle[] = m.getContent();
			double pos = new Double(angle[0]).doubleValue();
			getBrain().setHeading(pos);
		}
		
			
		
		ArrayList<WarPercept> nourritures = getBrain().getPerceptsResources();
		
		if (nourritures.size()>0)
		{
			getBrain().broadcastMessageToAgentType(WarAgentType.WarExplorer, Constants.foodHere, new Double(nourritures.get(0).getAngle()).toString());
		}
		
		ArrayList<WarPercept> baseEnnemie = getBrain().getPerceptsEnemiesByType(WarAgentType.WarBase);
		
		if (baseEnnemie.size()>0)
		{
			...
			//getBrain().broadcastMessageToAgentType(WarAgentType.WarBase, Constants.enemyBaseHere, new Double(baseEnnemie.get(0).getAngle()).toString());
		}
