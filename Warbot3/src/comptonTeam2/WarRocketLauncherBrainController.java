...
public class WarRocketLauncherBrainController extends WarRocketLauncherAbstractBrainController {
	...
	static final double rayonBase = 400.0; 
	static final double rayonBaseEnnemy = 400.0; 
	...
	public String action() {
		...
		boolean baseAttaque = false;
		boolean baseEnnemyTrouve = false;
		...
		WarMessage mess = this.getMessageAboutEnemyBase();
			
		if (mess!=null)
		{
			CoordPolar p = getBrain().getIndirectPositionOfAgentWithMessage(mess);
			
			if (p!=null)
			{
				if (p.getDistance() < rayonBaseEnnemy)
				{
					baseEnnemyTrouve = true;
		...
		if (!baseAttaque && !baseEnnemyTrouve)
		{
			if(!iAbleToFireBase)
...
//		
	...
	private void attackExplorer() {
		...
		getBrain().setDebugStringColor(Color.blue);
		getBrain().setDebugString("Attack explorers");
		
		ArrayList<WarPercept> percept = getBrain().getPerceptsEnemiesByType(WarAgentType.WarExplorer);
