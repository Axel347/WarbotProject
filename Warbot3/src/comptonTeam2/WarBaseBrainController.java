...
public class WarBaseBrainController extends WarBaseAbstractBrainController {
	...
	private void estAttaque()
	{
		ArrayList<WarPercept> ennemieDetected = getBrain().getPerceptsEnemiesByType(WarAgentType.WarRocketLauncher);
		if (ennemieDetected.size() > 0)
		{
			if (getBrain().getHealth() < WarBase.MAX_HEALTH)
			{
				String content[] = new String[0];
				getBrain().broadcastMessageToAgentType(WarAgentType.WarRocketLauncher,Constants.baseIsAttack,content);
			}
		}
	}
	
	private void envoyerMessageExplorer()
	{
		getBrain().broadcastMessageToAgentType(WarAgentType.WarExplorer, Constants.here, "");
	}
}
